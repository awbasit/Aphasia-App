...
function listeningSim1DocReady(nextUrl) {
  ...
  $(".next").click(function () {
    ...
    if (CURRENT_QUESTION <= questionsData.questions.length) {
      ...
      if (CURRENT_QUESTION == questionsData.questions.length) {
        $(".next").children().html("Next  Exercise");
      }
...
function readingSim2DocReady() {
  ...
  $(".start-button").click(function () {
    ...
    $(".task-read")
      ...
      .queue(function () {
        $("#directions").html(
          "Answer the questions by clicking on the best answer."
        );
        $(".task-read").hide();
        $("#answers").fadeIn("slow");
...
function writingSim1DocReady() {
  ...
  $("#sim-input").keyup(function () {
    ...
    var correct_word =
      questionsData.questions[CURRENT_QUESTION - 1].correct_word;
    var confused_word =
      questionsData.questions[CURRENT_QUESTION - 1].confused_word;
...
function writingSim3DocReady() {
  ...
  $("#sim-input-long").keyup(function () {
    ...
    var newtext = text.replace(
      /\b(the|a|an|was|were|are|is|am|have|has|been|on|under|below|above|on|at|off|of|)\b|/gi,
      ""
    );
...
function speakingSim1DocReady(nextUrl) {
  ...
  $(".next").click(function () {
    ...
    if (CURRENT_QUESTION <= questionsData.questions.length) {
      ...
      jwplayer().load({
        file: questionsData.questions[CURRENT_QUESTION - 1].audio_url,
      });
